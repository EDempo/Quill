...
int editor_row_reverse_conversion(erow *row, int rx) {
  int curr_rx = 0, cx;
...
void editor_process_keypress(void) {
  ...
  case '\r':
    E.mode == 'i' ? editor_insert_newline() : editor_move_cursor(ARROW_DOWN);
    break;

  case BACKSPACE:
    E.mode == 'i' ? editor_delete_char() : editor_move_cursor(ARROW_LEFT);
    break;
...
int main(int argc, char *argv[]) {
  ...
  editor_set_status_message("HELP: Ctrl-S to save | Ctrl-Q to quit | Ctrl-F to search");
  while (1) {
    editor_refresh_screen();
